...
Lx = nx  # Need M=I for now so use h=1
mesh = IntervalMesh(nx, Lx)
x, = SpatialCoordinate(mesh)

degree = options.getInt("fs_degree", 0)
variant = options.getString("fs_variant", "integral")  # integral gives us diagonal M
V = FunctionSpace(mesh, "DG", degree, variant=variant)

q = Function(V)
v = TestFunction(V)

u = as_vector([Constant(1.8)])
...
t = Constant(0)
dt = Constant(options.getReal("dt", 0.4))
